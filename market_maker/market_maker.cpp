...
Market_Maker::Market_Maker(std::string initFilePath, std::string initchosen, std::string logPath)
{
...
void Market_Maker::setConnStatus(bool connStatus)
{
    std::lock_guard<std::mutex> lckConnect(mutex_);
	connStatus_ = connStatus;
}

bool Market_Maker::getConnStatus()
{
    std::lock_guard<std::mutex> lckConnect(mutex_);
	return connStatus_;
}

void Market_Maker::socketini()
{
    ip->readLine();
...
void Market_Maker::sendMsg(std::string str)
{
    try
    {
...
void Market_Maker::heartbeatSending()
{
    while(!exit_flag_)
    {
        sendMsg("<3&");
...
void Market_Maker::startTradingProcess()
{
    while(!exit_flag_)
    {
        try
        {
            if(priceNow*0.9 == targetBuyPrice)
            {
                // sendMsg("87|313542|60|1|1|1|KKC|&");
                // sendMsg("87|313542|64|2|1|1|KKC|&");
                // send buy order
            }
            else if(priceNow*1.1 == targetSellPrice)
            {
                std::this_thread::sleep_for(std::chrono::seconds(10));
            }
...
void Market_Maker::msgHandler(std::string msg)
{
    if(msg.substr(0,2) == "<3")
		return;
    try
...
int main()
{
    Market_Maker *cl = new Market_Maker("./doc/settings.ini", "TraderServer", "./log/");
}
